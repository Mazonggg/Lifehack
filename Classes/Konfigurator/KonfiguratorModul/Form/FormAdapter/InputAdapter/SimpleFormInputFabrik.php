...
class SimpleFormInputFabrik {
    ...
    public static function erzeugeFormInput($type, $name, $daten) {
        if ($type == self::NUMBER) {
            $formInput = new NumberFormInput($name, $daten[self::INHALT]);
            ...
            $formInput = new TextareaFormInput($name, $daten[self::INHALT]);
        } elseif ($type == self::SELECT) {
            $formInput = new SelectFormInput($name, $daten[self::INHALT]);
            $formInput->setOptionen($daten[self::OPTIONEN]);
            $formInput->setSelected($daten[self::SELECTED]);
            $formInput->setLabel($daten[self::LABEL]);
        } elseif ($type == self::AUSWAHLBILD) {
            $formInput = new AuswahlbildFormFormInput($name, $daten[self::INHALT]);
            $formInput->setOptionen($daten[self::OPTIONEN]);
            $formInput->setSelected($daten[self::SELECTED]);
            $formInput->setLabel($daten[self::LABEL]);
        } elseif ($type == self::BOOLEAN) {
            $formInput = new BooleanFormFormInput($name, $daten[self::INHALT]);
            $formInput->setSelected($daten[self::SELECTED]);
            $formInput->setLabel($daten[self::LABEL]);
        } elseif ($type == self::NEUBUTTON) {
            $formInput = new NeuButtonFormInput($name, $daten[self::INHALT]);
            $formInput->setId($daten[self::ID]);
        } elseif ($type == self::HIDDEN) {
            $formInput = new HiddenFormInput($name, $daten[self::INHALT]);
            $formInput->setId($daten[self::ID]);
        } elseif ($type == self::AUSWAHL_BUTTON) {
            $formInput = new AuswahlButtonFormFormInput($name, $daten[self::INHALT]);
